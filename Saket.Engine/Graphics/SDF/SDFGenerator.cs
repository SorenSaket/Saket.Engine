...
namespace Saket.Engine.Graphics.SDF
{
    ...
    {  
        ...
        protected float[] ContourSd;

        /// <summary>
        /// Windings for each contour in the shape
        /// </summary>
        ...
        public void GenerateSDF(Settings settings, Color[] output, int outputWidth, Shape shape, float range, Vector2 scale, Vector2 translation)
        {
            this.shape = shape;
            ...
            {
                var row = shape.InverseYAxis ? height - y - 1f : y;
        ...
        /// <param name="shape"></param>
        /// <returns></returns>
        private void Generate_EdgeColors(Shape shape, List<Color> edgecolors)
        {
            foreach (var spline in shape)
            {
                // TODO ADD threshhold
                spline.GetCorners(coners, 0);
        ...
        private void ProcessMultiCornerContour(Spline2D contour)
        {
            ...
            {
                var index = (start + i) % m;
                if (spline + 1 < cornerCount && coners[spline + 1] == index)
                {
                    ++spline;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected float ComputeSDFPixel(ref InstanceContext ctx)
        {
            double dummy = 0;
        ...
        */
        ...
        /// <param name="g"></param>
        /// <param name="b"></param>
        private void MsdfScanContourEdges(Spline2D contour, Vector2 p, ref CurvePoint r, ref CurvePoint g, ref CurvePoint b)
        {
