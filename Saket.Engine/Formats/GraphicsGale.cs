...
/// </summary>
public static class GraphicsGale
{
...
public class GaleObject:IDisposable
{
    ...
    public ColorPalette Palette {
        get {
            ...
            if(palette == null) {
                using(var bmp = GraphicsGale.GetBitmap(Handle, 0, 0)) {
                    palette = bmp.Palette;
                }
            }
            return palette;
    ...
    public GaleObject(string path) {
        Handle = GraphicsGale.Open(path);

        if(Handle.ToInt64() == 0) {
            switch(GraphicsGale.GetLastError()) {
        ...
        Width = GraphicsGale.GetWidth(Handle);
        Height = GraphicsGale.GetHeight(Handle);

        SinglePalette = GraphicsGale.IsSinglePalette(Handle);
        BackgroundColor = GraphicsGale.GetBackgroundColor(Handle);

        DisabledBottomLayerTransparency = GraphicsGale.IsBottomLayerTransparencyDisabled(Handle);

        BitsPerPixel = GraphicsGale.GetBitsPerPixel(Handle);
        IndexedColor = BitsPerPixel <= 8;

        FrameCount = GraphicsGale.GetFrameCount(Handle);
        _ZeroDigitFormat = new String('0', FrameCount.ToString().Length);

    }

    public void Dispose() {
        GraphicsGale.Close(Handle);
        frames = null;
        Handle = IntPtr.Zero;
    }
}
...
public class Frame
{
    ...
    public ColorPalette Palette {
        get {
            ...
            if(palette == null) {
                using(var bmp = GraphicsGale.GetBitmap(galeObject.Handle, frameIndex, 0)) {
                    palette = bmp.Palette;
                }
            }
            return palette;
    ...
    internal Frame(GaleObject galeObject, int frameIndex) {
        ...
        InternalName = GraphicsGale.GetFrameName(galeObject.Handle, frameIndex);
        ...
        TransparentColor = GraphicsGale.GetFrameTransparentColor(galeObject.Handle, this.frameIndex);
        DisposalType = GraphicsGale.GetFrameDisposalType(galeObject.Handle, this.frameIndex);
        Delay = GraphicsGale.GetFrameDelay(galeObject.Handle, this.frameIndex);

        LayerCount = GraphicsGale.GetLayerCount(galeObject.Handle, frameIndex);
    }

    public Bitmap CreateBitmap() {
        var bmp = GraphicsGale.GetBitmap(galeObject.Handle, frameIndex, GraphicsGale.ALL_LAYERS);
...
public class Layer
{
    ...
    internal Layer(GaleObject galeObject, int frameIndex, int layerIndex) {
        ...
        Name = GraphicsGale.GetLayerName(galeObject.Handle, frameIndex, layerIndex);

        Visible = GraphicsGale.IsLayerVisible(galeObject.Handle, frameIndex, layerIndex);
        AlphaChannelEnabled = GraphicsGale.IsAlphaChannelEnabled(galeObject.Handle, frameIndex, layerIndex);

        TransparentColor = GraphicsGale.GetLayerTransparentColor(galeObject.Handle, frameIndex, layerIndex);
        Opacity = GraphicsGale.GetLayerOpacityLevel(galeObject.Handle, frameIndex, layerIndex);
    }

    public Bitmap CreateBitmap() {
        var bmp = GraphicsGale.GetBitmap(galeObject.Handle, frameIndex, layerIndex);
        bmp.MakeTransparent();
        return bmp;
    }
