...
namespace Saket.Engine.Typography.TrueType
{
    class Interpreter {
        ...
        public void SetControlValueTable (FUnit[] cvt, float scale, float ppem, byte[] cvProgram) {
            ...
            this.scale = scale;
            this.ppem = (int)System.Math.Round(ppem);
            zp0 = zp1 = zp2 = points;
            state.Reset();
            stack.Clear();
            ...
            while (!stream.Done) {
                ...
                switch (opcode) {
                    ...
                    case OpCode.MIAP1:
                        {
                            ...
                            if (opcode == OpCode.MIAP1) {
                                ...
                                if (MathF.Abs(distance - currentPos) > state.ControlValueCutIn)
                                    distance = currentPos;
                    ...
                    case OpCode.ISECT:
                        {
                            ...
                            if (System.Math.Abs(den) <= Epsilon) {
                    ...
                    case OpCode.MUL:
                        ...
                        break;
                    case OpCode.ABS: stack.Push(System.Math.Abs(stack.Pop())); break;
                    ...
                    case OpCode.MAX: stack.Push(System.Math.Max(stack.Pop(), stack.Pop())); break;
                    case OpCode.MIN: stack.Push(System.Math.Min(stack.Pop(), stack.Pop())); break;
        ...
        void OnVectorsUpdated () {
            ...
            if (System.Math.Abs(fdotp) < Epsilon)
                fdotp = 1.0f;
        }
        ...
        void MoveIndirectRelative (int flags) {
            ...
            if (System.Math.Abs(cvt - state.SingleWidthValue) < state.SingleWidthCutIn) {
                if (cvt >= 0)
            ...
            if (state.AutoFlip && System.Math.Sign(originalDistance) != System.Math.Sign(cvt))
                cvt = -cvt;
            ...
            if ((flags & 0x4) != 0) {
                ...
                if (zp0.IsTwilight == zp1.IsTwilight && System.Math.Abs(cvt - originalDistance) > state.ControlValueCutIn)
            ...
            if ((flags & 0x8) != 0) {
                if (originalDistance >= 0)
                    distance = System.Math.Max(distance, state.MinDistance);
                else
                    distance = System.Math.Min(distance, -state.MinDistance);
            }
        ...
        void MoveDirectRelative (int flags) {
            ...
            if (System.Math.Abs(originalDistance - state.SingleWidthValue) < state.SingleWidthCutIn) {
                if (originalDistance >= 0)
        ...
        float Round (float value) {
            switch (state.RoundState) {
                case RoundMode.ToGrid: return value >= 0 ? (float)System.Math.Round(value) : -(float)System.Math.Round(-value);
                case RoundMode.ToHalfGrid: return value >= 0 ? (float)System.Math.Floor(value) + 0.5f : -((float)System.Math.Floor(-value) + 0.5f);
                case RoundMode.ToDoubleGrid: return value >= 0 ? (float)(System.Math.Round(value * 2, MidpointRounding.AwayFromZero) / 2) : -(float)(System.Math.Round(-value * 2, MidpointRounding.AwayFromZero) / 2);
                case RoundMode.DownToGrid: return value >= 0 ? (float)System.Math.Floor(value) : -(float)System.Math.Floor(-value);
                case RoundMode.UpToGrid: return value >= 0 ? (float)System.Math.Ceiling(value) : -(float)System.Math.Ceiling(-value);
                case RoundMode.Super:
                case RoundMode.Super45:
                    float result;
                    if (value >= 0) {
                        result = value - roundPhase + roundThreshold;
                        result = (float)System.Math.Truncate(result / roundPeriod) * roundPeriod;
                        result += roundPhase;
                        if (result < 0)
                            result = roundPhase;
                    }
        ...
        static int FloatToF2Dot14 (float value) => (int)(uint)(short)System.Math.Round(value * 16384.0f);
        static float F26Dot6ToFloat (int value) => value / 64.0f;
        static int FloatToF26Dot6 (float value) => (int)System.Math.Round(value * 64.0f);

        unsafe static float* GetPoint (byte* data, int index) => (float*)(data + sizeof(PointF) * index);

        static readonly float Sqrt2Over2 = (float)(System.Math.Sqrt(2) / 2);
