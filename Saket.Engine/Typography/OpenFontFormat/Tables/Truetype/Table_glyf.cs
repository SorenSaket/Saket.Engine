...
namespace Saket.Engine.Filetypes.Font.OpenFontFormat.Tables
{
    ...
    public class Table_glyf : Table
    {
        ...
        public struct SimpleGlyph
        {
            ...
            {
                reader.LoadBytes(2 * numberOfContours + 2);
                endPtsOfContours = new ushort[numberOfContours];
                for (int i = 0; i < numberOfContours; i++)
                {
                    reader.ReadUInt16(ref endPtsOfContours[i]);
                }

                reader.ReadUInt16(ref instructionLength);

                reader.LoadBytes(instructionLength);
                ...
                int pointCount = endPtsOfContours[^1] + 1;

                // Read Flags

                // Over allocate flags. The actual number of flags is less than pointCount
                flags = new Flags[pointCount];
                ...
                {
                    // TODO rework serialization lib to support enums	
                    reader.LoadBytes(1);
                    reader.ReadUInt8(ref flagbyte);
                    Flags flag = (Flags)flagbyte;
                    flags[flagIndex++] = flag;

                    // if Repeat is set, this flag data is repeated n more times
                    if ((flag & Flags.REPEAT_FLAG) != 0)
                    {
                        reader.LoadBytes(1);
                        reader.ReadUInt8(ref repeatCount);
                ...
                byte value_short = 0;
                // Read xDeltas
                xCoordinates = new Int16[pointCount];
                for (int i = 0; i < pointCount; i++)
                {
                    Flags flag = flags[i];
                    
                    // The coordinate is a uint8
                    if ((flag & Flags.X_SHORT_VECTOR) != 0)
                    {
                        reader.LoadBytes(1);
                        reader.ReadUInt8(ref value_short);

                        // The coordinate is negative
                        if ((flag & Flags.X_IS_SAME_OR_POSITIVE_X_SHORT_VECTOR) == 0)
                        {
                            xCoordinates[i] = (Int16)(-value_short);
                        }
                        else
                        {
                            xCoordinates[i] = (Int16)(value_short);
                    ...
                    else if ((flag & Flags.X_IS_SAME_OR_POSITIVE_X_SHORT_VECTOR) == 0)
                    {
                        reader.LoadBytes(2);
                        reader.ReadInt16(ref xCoordinates[i]);
                    }
                }

                // Read yDeltas
                yCoordinates = new Int16[pointCount];
                for (int i = 0; i < pointCount; i++)
                {
                    Flags flag = flags[i];

                    // The coordinate is a uint8
                    if ((flag & Flags.Y_SHORT_VECTOR) != 0)
                    {
                        reader.LoadBytes(1);
                        reader.ReadUInt8(ref value_short);

                        // The coordinate is negative
                        if ((flag & Flags.Y_IS_SAME_OR_POSITIVE_Y_SHORT_VECTOR) == 0)
                        {
                            yCoordinates[i] = (Int16)(-value_short);
                        }
                        else
                        {
                            yCoordinates[i] = (Int16)(value_short);
                    ...
                    else if ((flag & Flags.Y_IS_SAME_OR_POSITIVE_Y_SHORT_VECTOR) == 0)
                    {
                        reader.LoadBytes(2);
                        reader.ReadInt16(ref yCoordinates[i]);
                    }
                }
            }
        }
